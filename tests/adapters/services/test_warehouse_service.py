...
from tests.fixtures.fixtures import create_warehouse, create_product, create_palette, populate_movements

@pytest.mark.django_db
class TestWarehouseService:

    @pytest.fixture(autouse=True)
    def setup(self, create_warehouse, create_product, create_palette):
    ...
    def test_move_product_success(self):
        ...
        movement_log = StockMovementLog.objects.get(from_location=self.palette_a, to_location=self.palette_b)
        assert movement_log.quantity == 50
        assert movement_log.movement_type == 'MOVE'

    def test_move_product_insufficient_stock(self):
        with pytest.raises(ValueError, match="La quantité demandée dépasse le stock disponible"):
    ...
    def test_move_product_to_occupied_location(self, create_product):
        ...
        with pytest.raises(ValueError, match="L'emplacement de destination contient déjà un autre produit"):
    ...
    def test_get_stock_levels(self):
        ...
        assert stock_levels[self.palette_a.name] == 100
        assert stock_levels[self.palette_b.name] == 100
